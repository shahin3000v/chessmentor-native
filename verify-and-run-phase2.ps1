...
$logDirectory = Join-Path $repo 'artifacts\verification\phase2'
...
function Invoke-DotNetGate {
    ...
    if ($exitCode -ne 0) {
        throw "Phase 2 $Name failed with exit code $exitCode. Full log: $logPath"
    }
}
...
if ($LASTEXITCODE -ne 0 -or -not $sdkVersion.StartsWith('10.')) {
    throw "Phase 2 requires the .NET 10 SDK. Detected version: $sdkVersion"
}
...
try {
    ...
    Write-Host "`nPhase 2 build and tests passed." -ForegroundColor Green
    Write-Host "Logs: $logDirectory" -ForegroundColor Green

    if (-not $NoLaunch) {
        Write-Host 'Launching native Viewer 1...' -ForegroundColor Green
        & dotnet run --project $desktopProject -c $Configuration --no-build --no-restore
        if ($LASTEXITCODE -ne 0) {
            throw "Viewer 1 exited with code $LASTEXITCODE."
        }
    }
}
catch {
    Write-Host "`nPHASE 2 ACCEPTANCE FAILED" -ForegroundColor Red
